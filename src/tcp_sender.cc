...
void TCPSender::push( const TransmitFunction& transmit )
{
  ...
  // 循环执行: 1.组装报文到大小上限, 2.发送报文, 循环退出条件: 已发送但未确认的包的大小总和达到窗口上限, 或者别的什么错误
  for(std::string payload {}; cnt_seq_in_flight_ < window_size_; payload.clear())
  {
    std::string_view content = rdr.peek();
    if(content.empty() && sent_syn_ && !read_fin_)
    {
      return;// input_流空了, 并且已经发送过syn, 并且input_未结束, 那么就啥也不用发送, 直接return
    }
    const size_t upper_size = TCPConfig::MAX_PAYLOAD_SIZE;
    const uint16_t wnd_size = window_size_ == 0 ? 1 : window_size_; //当窗口大小为0的时候, 需要将假装是1, 否则永远无法知道接收方的状态
    const uint64_t syn_size = sent_syn_ ? 0:1; // 如果没有发送过syn_flag, 那么计算报文大小的时候则需要将syn_flag的大小计入
    ...
    {
      uint64_t expected_size = min(wnd_size - syn_size - payload.size() - cnt_seq_in_flight_, upper_size - payload.size() - syn_size);
    ...
    if(read_fin_)
    {
      ...
      if(payload.size() + !sent_syn_ + 1 < upper_size && payload.size() + !sent_syn_ + cnt_seq_in_flight_ + 1 < wnd_size)
      {
        fin_flag = true;
      }
    }    
...
{
  return {
    .seqno = seqno, .SYN = syn, .payload = std::move( payload ), .FIN = fin, .RST = input_.has_error() };
}
